...
@section('title')
    <h1>Fecha: {{$fecha->numero_fecha}} @if($fecha->es_play_off==1)(PlayOff)@endif - <small>Torneo: {{$fecha->Torneo->nombre_torneo}} - {{$fecha->Torneo->TipoTorneo->nombre_tipo_torneo}} - Zona: {{$fecha->Zona->nombre}} </small></h1>
@endsection
                        ...
                        <table id="editar"  class=" table table-bordered table-condensed table-hover">
                            ...
                            @foreach($fecha->ListPartidos as $partido)
                                <tr >
                                    ...
                                    <td>{{$partido->goles_local}} - {{$partido->goles_visitante}}
                                        @if($fecha->es_play_off ==1 and $partido->goles_local == $partido->goles_visitante and isset($partido->goles_local)  )
